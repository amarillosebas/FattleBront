...
namespace WhiteCat.Editor
{
	partial class ScriptGenerater
	{
		...
		{
			string path = string.Format("{0}/{1}.cs", EditorAsset.activeDirectory, className);
...
@"	}
		...
		static string GetVariableName(string originalName, string fallbackName, List<string> list)
		{
			...
			for (int i = 0; i < chars.Count; i++)
			{
				...
				if (i == 0) invalid = (c != '_' && !Text.IsEnglishLetter(c));
				else invalid = (c != '_' && !Text.IsDigit(c) && !Text.IsEnglishLetter(c));
