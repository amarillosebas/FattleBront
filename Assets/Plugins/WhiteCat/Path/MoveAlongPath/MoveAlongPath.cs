		...
		protected sealed override void Editor_OnInspectorGUI()
		{
			...
			if (_path)
			{
				...
				if (_keyframeLists.Count > 0)
				{
					...
					rect.height = 1f;
					Color lineColor = EditorUI.defaultContentColor * 0.5f;
					...
					EditorGUI.LabelField(left, "Keyframe List", EditorUI.centeredBoldLabelStyle);
					EditorGUI.LabelField(right, "Target Component", EditorUI.centeredBoldLabelStyle);
					...
					for (int i = 0; i < _keyframeLists.Count; i++)
					{
						...
						EditorUI.BeginGUIColor(keyframeList.personalizedColor);
