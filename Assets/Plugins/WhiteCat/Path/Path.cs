	...
	public abstract partial class Path : ScriptableComponentWithEditor
	{
		...
		public float worldScale
		{
			...
			set
			{
				if (value < 0f)
				{
					_worldScale = Mathf.Clamp(value, -Math.Million, -Math.OneMillionth);
				}
				else
				{
					_worldScale = Mathf.Clamp(value, Math.OneMillionth, Math.Million);
				}
			}
		}
		...
		void Awake()
		{
			if (Utilities.IsNullOrEmpty(_localSegments)) Reset();
		}
